...
def test_extract_motion_templates_can_drive_from_face_crop_video(tmp_path: Path, monkeypatch) -> None:
    ...
    for frame_name in ("000000.png", "000001.png"):
        (face_crop_dir / frame_name).write_text("png", encoding="utf-8")
    ...
    monkeypatch.setattr("avagen.renderers.motion_template.shutil.which", lambda name: "/usr/bin/ffmpeg")

    def fake_ffmpeg_run(command, capture_output, text, check):  # noqa: ANN001
        Path(command[-1]).write_text("mp4", encoding="utf-8")
        return SimpleNamespace(returncode=0, stderr="")

    monkeypatch.setattr("avagen.renderers.motion_template.subprocess.run", fake_ffmpeg_run)

    seen_driving_paths: list[Path] = []

    def fake_runner(config, dry_run=False):
        seen_driving_paths.append(config.driving_path)
    ...
    result = extract_motion_templates(
        MotionTemplateExtractionConfig(
            ...
            overwrite=True,
            driving_source="face_crop_video",
        ),
        runner=fake_runner,
    )

    assert seen_driving_paths
    assert seen_driving_paths[0].name == "clip_a_face_crops.mp4"
    assert (clip_dir / "motion_template.pkl").exists()
